<x-app-layout>
    ...
    <form action="{{route('pios.store')}}" method="POST">
        @csrf 
        <textarea name="title" placeholder="{{__('Titulo de tu problema')}}" 
        class="block border-gray-300 focus:border-indigo-300 focus:ring 
        focus-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
        {{old('title')}}
        </textarea>

        <textarea name="message" placeholder="{{__('En que piensas amigo?')}}" 
